...
export default function ServiceCategoriesPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-12">
      <h1 className="mb-8 text-2xl font-bold text-gray-900">Service Categories</h1>
      ...
      <Card>
        ...
        <CardContent>
          <form onSubmit={handleCreateCategory} className="flex flex-col gap-4 sm:flex-row sm:items-end">
            <Input label="Name" value={name} onChange={(e) => setName(e.target.value)} required placeholder="e.g. Painting" />
            <Input label="Description" value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Optional description" />
            <Button type="submit" isLoading={creating} className="lg:w-2/5">Add Category</Button>
          </form>
        </CardContent>
        <CardContent>
          <ul className="flex flex-col gap-2">
            {categories.map((category) => (
              <li key={category.id} className="rounded-lg border border-gray-200 px-3 py-2 text-sm">
